...
struct InertialInitializerOptions {
  ...
  bool init_dyn_use_drt = false;
  double init_drt_vis_weight = 20.0;
  ...
  void print_and_load_initializer(const std::shared_ptr<ov_core::YamlParser> &parser = nullptr) {
    ...
    if (parser != nullptr) {
      ...
      parser->parse_config("init_dyn_use_drt", init_dyn_use_drt, false);
      parser->parse_config("init_drt_vis_weight", init_drt_vis_weight, false);
    ...
    PRINT_DEBUG("  - init_dyn_use_drt: %d\n", init_dyn_use_drt);
    PRINT_DEBUG("  - init_drt_vis_weight: %.1f\n", init_drt_vis_weight);
